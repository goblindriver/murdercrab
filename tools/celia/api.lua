...
function api.run()
	...
	-- murdercrab: host-side bootstrap hook (seeds RNG; no-op if file absent).
	-- Keeps upstream Celia pull to a ~5-line diff in this file only.
	local ok_bs, bootstrap = pcall(require, "murdercrab_bootstrap")
	if ok_bs and bootstrap.pre_init then bootstrap.pre_init(pico8.cart, api) end
